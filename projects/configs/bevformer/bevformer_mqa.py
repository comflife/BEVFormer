# BEVFormer-MQA: Language-guided BEVFormer for Multi-task QA
# Based on bevformer_small.py with language modules added
#
# Key differences from bevformer_small:
# 1. Uses BEVFormerMQA detector instead of BEVFormer
# 2. Adds TextEncoder (BERT), PriorHead, and MQAHead
# 3. Uses NuScenesMQADataset with question-answer annotations
# 4. Backbone/neck frozen, only language modules trained
...
model = dict(
    ...
    # MQA Head (multi-task prediction)
    mqa_head_cfg=dict(
        type='MQAHead',
        in_channels=_dim_,
        text_channels=_dim_,
    ...
    # Prior Supervision (optional, helps prevent prior collapse)
    ...
    loss_weight_distance=0.5,
    loss_weight_location=0.5,
