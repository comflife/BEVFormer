# ---------------------------------------------
# PerceptionTransformer with Image-level DiffFP Support
# ---------------------------------------------
# Passes DiffFP masks through to encoder for sparse spatial attention
# ---------------------------------------------
...
class PerceptionTransformerDiffFP(PerceptionTransformer):
    """PerceptionTransformer with Image-level DiffFP support.
    
    Extends the base PerceptionTransformer to pass DiffFP masks
    through to the encoder layers, enabling sparse computation
    ...
    def get_bev_features(
            ...
            diff_fp_masks=None,
            **kwargs):
        """Obtain BEV features with DiffFP support.
        
        Args:
            mlvl_feats: Multi-level features [B, N, C, H, W]
            bev_queries: BEV query embeddings
            bev_h, bev_w: BEV grid size
            grid_length: Grid cell size
            bev_pos: BEV positional encoding
            prev_bev: Previous BEV features
            diff_fp_masks: List of DiffFP mask dicts per level
